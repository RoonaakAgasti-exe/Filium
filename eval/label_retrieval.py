...
Interactive tool for hand-labeling retrieval quality. For each question in
eval_questions.json, runs retrieval, shows you the top-k chunks, and asks
you to mark each one relevant or not. This manual labeling is the tedious
but essential part — it's what turns "the retrieval looks fine" into an
actual, defensible precision number.

Labels are saved incrementally, so you can stop partway through and resume
later without re-labeling what you've already done.
...
def main():
    ...
    try:
        for q in questions:
            ...
            save_labels(labels)  # save after every question, not just at the end
