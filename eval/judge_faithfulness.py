...
Runs each eval question through the real RAG pipeline (answer_query.py),
then asks an LLM to judge whether the generated answer only used
information present in the retrieved excerpts, or whether it added
outside claims not supported by them (hallucination).

IMPORTANT: this is LLM-graded, not human-graded. Say so explicitly in
your README — it's a legitimate and common technique, but it is not the
same rigor as human evaluation, and overstating it undermines the
credibility of your other, human-labeled numbers.
