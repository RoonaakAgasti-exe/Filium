...
if config.IS_PRODUCTION and SECRET_KEY == "dev-only-change-this-in-production":
    # Refusing to boot is the right call here: with the default secret,
    # anyone who has read this repo can mint a valid token for any user.
    raise RuntimeError(
        "JWT_SECRET_KEY must be set to a real random value when ENVIRONMENT=production"
    )
if SECRET_KEY == "dev-only-change-this-in-production":
...
# passlib is unmaintained and its bcrypt backend detection breaks on
# bcrypt>=4.1 (which removed the internal `__about__` attribute passlib
# checks for) — confirmed hitting this exact crash while building this
# backend. Calling bcrypt directly sidesteps the issue entirely.


def hash_password(plain_password: str) -> str:
    # bcrypt has a hard 72-byte input limit — truncate rather than error,
    # since a user's actual password rarely exceeds this in practice.
