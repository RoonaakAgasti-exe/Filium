...
def read_portfolio(user_id: int = Depends(get_current_user_id), conn: PgConnection = Depends(get_conn)):
    return _load_portfolio(conn, user_id)

@router.get("/history")
def read_portfolio_history(user_id: int = Depends(get_current_user_id), conn: PgConnection = Depends(get_conn)):
    cur = conn.cursor()
    try:
        cur.execute(
...
def read_transactions(limit: int = Query(default = 20, ge = 1, le = 200), user_id: int = Depends(get_current_user_id), conn: PgConnection = Depends(get_conn)):
    cur = conn.cursor()
    try:
        cur.execute(
            """
...
def read_analytics(user_id: int = Depends(get_current_user_id), conn: PgConnection = Depends(get_conn)):
    pass
...
def read_vs_benchmark(user_id: int = Depends(get_current_user_id), conn: PgConnection = Depends(get_conn)):
    pass
    cur = conn.cursor()
    try:
        cur.execute(
    ...
    if not first_value:
        return {"portfolio_return": None, "benchmark_return": None, "message": "First snapshot has zero value; cannot compute a return."}
