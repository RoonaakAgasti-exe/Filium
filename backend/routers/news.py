...
def get_news(ticker: str, limit: int = Query(default=30, ge=1, le=200),
    ...
    try:
        cur.execute(
            ...
            "ORDER BY published_date DESC, id DESC LIMIT %s",
            (ticker, limit),
        )
        rows = cur.fetchall()
    finally:
...
def sentiment_timeline(ticker: str, days: int = Query(default=180, ge=1, le=1825),
    ...
    if not rows:
        raise HTTPException(
            ...
            detail=f"No price or sentiment data for {ticker}. Run "
                   f"ingestion/fetch_prices.py and ingestion/fetch_news.py first.",
        )
