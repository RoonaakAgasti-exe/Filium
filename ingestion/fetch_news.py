...
Fills `news_articles` with recent headlines per ticker. This is the raw
input to FinBERT scoring (`ml/sentiment.py --source news`), which in turn
produces the `sentiment_scores` rows that separate the augmented LSTM
from the baseline one. No news, no sentiment feature, no ablation.

Headlines are stored raw and unscored (`sentiment_score` stays NULL until
the sentiment job runs). That split is deliberate: swapping the sentiment
model or fixing an aggregation bug then means re-scoring what's already
on disk instead of re-fetching from a rate-limited API.

Three sources, tried in this order:

    1. Finnhub    (FINNHUB_API_KEY) — real per-company news, 1y of history
    2. NewsAPI    (NEWS_API_KEY) — broad coverage, free tier is last ~30 days
    3. Google News RSS (no key) — keyless fallback so a fresh clone works

Usage:
    python fetch_news.py AAPL
    python fetch_news.py AAPL MSFT --days 90
    python fetch_news.py --watchlisted
"""
...
#   {published_date: date, headline: str, summary: str|None,
...
def fetch_from_newsapi(ticker: str, start: date, end: date) -> list[dict]:
    ...
    resp = requests.get(
        ...
        params={
            # Bare tickers are terrible search terms — "A", "IT" and "ALL"
            # are real symbols. Anchoring on the word "stock" costs some
            # recall and buys a lot of precision.
...
def fetch_from_rss(ticker: str, start: date, end: date) -> list[dict]:
    """
    Google News RSS — keyless, so `python fetch_news.py AAPL` does
    something on a clone with an empty .env.

    Honest about what it is: an RSS search feed returns roughly the last
    100 items regardless of the date range asked for, so `--days 365`
    against this source will not backfill a year. Use Finnhub for real
    history.
    """
    resp = requests.get(
        "https://news.google.com/rss/search",
        params={"q": f"{ticker} stock", "hl": "en-US", "gl": "US", "ceid": "US:en"},
        headers={"User-Agent": "FinCopilot/1.0 (news ingestion)"},
        timeout=REQUEST_TIMEOUT,
    )
    resp.raise_for_status()
    ...
    for item in root.iterfind(".//item"):
        ...
        # headline FinBERT scores is the headline, not headline+outlet.
...
def fetch_articles(ticker: str, start: date, end: date,
                   source: str = "auto") -> tuple[list[dict], str]:
...
def store_articles(conn, ticker: str, articles: list[dict]) -> int:
    """
    Inserts, skipping anything already stored.

    DO NOTHING, not DO UPDATE: an existing row may already carry a
    `sentiment_score`, and overwriting the row from a re-fetch would wipe
    that score and quietly leave the daily aggregate in `sentiment_scores`
    referring to articles that no longer look scored.
    """
    if not articles:
        return 0
    ...
    try:
        ...
        cur.executemany(
            ...
            """,
            [
                (ticker, a["published_date"], a["headline"], a["summary"],
                 a["url"], a["source"])
                for a in articles
            ],
        )
