...
import pytest
from backend.wallet import (InsufficientFundsError, InsufficientSharesError, buy_shares, ensure_wallet, get_cash_balance, get_holding, get_portfolio, save_daily_snapshot, sell_shares)
...
class FakeCursor:
    ...
    def execute(self, sql, params = None):
        ...
        if sql.startswith("SELECT") and "FROM wallets WHERE user_id" in sql:
            ...
            self._rows = [(ticker, h["shares"], h["avg_cost_basis"], self._db["sectors"].get(ticker)) for ticker, h in sorted(holdings.items())]
...
class TestSellShares:
    def test_realizes_profit_and_leaves_cost_basis_alone(self):
        ...
        assert result["realized_pl"] == pytest.approx(200.0)
        assert conn.cash == pytest.approx(600.0)
...
class TestRoundTrip:
    def test_buy_buy_sell_leaves_consistent_cash_and_basis(self):
        conn = FakeConn(cash = 10_000.0)
        buy_shares(conn, USER, "AAPL", 10, 100.0)
        buy_shares(conn, USER, "AAPL", 10, 140.0)
        result = sell_shares(conn, USER, "AAPL", 5, 200.0)
...
class TestGetPortfolio:
    def test_values_holdings_and_totals_cash_plus_market_value(self):
        conn = FakeConn(cash = 1_000.0, holdings = {"AAPL":holding(10.0, 100.0), "MSFT":holding(5.0, 200.0)}, sectors = {"AAPL":"Technology", "MSFT":"Technology"})
        portfolio = get_portfolio(conn, USER, {"AAPL":120.0, "MSFT":180.0})
        assert portfolio["holdings_value"] == pytest.approx(2_100.0)
    ...
    def test_an_unpriced_holding_is_reported_without_dropping_the_rest(self):
        conn = FakeConn(cash = 500.0, holdings = {"AAPL":holding(10.0, 100.0), "OBSCURE":holding(3.0, 50.0)})
...
class TestWalletHelpers:
    ...
    def test_save_daily_snapshot_writes_the_three_value_columns(self):
        conn = FakeConn(cash = 1_000.0)
        portfolio = {"total_value":3_100.0, "cash_balance":1_000.0, "holdings_value":2_100.0}
