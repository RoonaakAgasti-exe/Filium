"""
Tests for the quote fallback chain in market_data.py.

The point of these is the no-Alpaca path. Alpaca needs a brokerage signup
that can take days, so the app is built to price a portfolio without one —
these pin down that promise, including the case where every network
source is dead and only a stored close is left.

Nothing here touches the network: each source is stubbed at the
...
class FakeCursor:
    """Just enough psycopg2 cursor to satisfy latest_close_from_db."""
...
def _clean_cache_and_config(monkeypatch):
    """
    Every test starts from a cold cache and a known config. Without the
    reset, a quote cached by an earlier test satisfies a later one before
    its stub is ever consulted, and the test passes for the wrong reason.
    """
...
class TestNoAlpacaAccount:
    ...
    def test_falls_through_to_yahoo_when_fmp_errors(self, monkeypatch):
        ...
        def fake_get(url, **kwargs):
            if "financialmodelingprep" in url:
                # FMP signals a bad key with HTTP 200 and an error body.
    ...
    def test_raises_only_when_nothing_at_all_can_price_it(self, monkeypatch):
        monkeypatch.setattr(
            market_data.requests, "get", lambda *a, **k: (_ for _ in ()).throw(ConnectionError())
        )
    ...
    def test_error_message_does_not_demand_a_brokerage_account(self, monkeypatch):
        """
        The old message told users to configure Alpaca, which is the one
        thing they cannot quickly do. It should point at ingestion instead.
        """
        monkeypatch.setattr(
            market_data.requests, "get", lambda *a, **k: (_ for _ in ()).throw(ConnectionError())
        )
...
class TestQuoteCache:
    ...
    def test_ttl_of_zero_disables_caching(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda *a, **k: (calls.append(1), FakeResponse(YAHOO_OK))[1],
        )
...
class TestBulkLookup:
    ...
    def test_unpriceable_ticker_is_omitted_not_fatal(self, monkeypatch):
        ...
        # FakeConn has no stored close either, so MSFT cannot be priced.
