"""
Tests for company_info.py.

The bug this module fixes was invisible rather than loud: every writer of
`companies` inserted the ticker as the name and left the sector NULL, so
`/portfolio/analytics` reported one "Unclassified" bucket at 100% weight
and looked like a feature that had been built and never worked.

So the cases that matter are the ones about *not* clobbering good data
(enrichment must be idempotent and must never blank a populated column)
and about the keyless tier, which is what keeps the sector breakdown
meaningful in a clone with an empty .env.
"""
...
class FakeCursor:
    ...
    def execute(self, sql, params=()):
        ...
        if normalised.startswith("SELECT TICKER FROM COMPANIES"):
            self._result = [
                (t,) for t, r in sorted(self._db.items())
                if not r["sector"] or not r["name"] or r["name"].upper() == t
            ]
            return
        ...
        if normalised.startswith("UPDATE COMPANIES"):
            ...
            if row is None:
                self.rowcount = 0
                return
            # Mirrors the COALESCE semantics of the real statement.
            if name is not None:
                row["name"] = name
            if not row["sector"]:
                row["sector"] = sector
...
class FakeConn:
    def __init__(self, companies):
        self.db = {
            t: {"name": v[0], "sector": v[1], "cik": None}
...
def no_fmp(monkeypatch):
    """The empty-.env case: no key, so only the seed tier is available."""
...
class TestDoesNotClobber:
    ...
    def test_enrichment_does_not_commit_on_its_own(self, no_fmp):
        """It rides along with the caller's transaction — see the docstring."""
        conn = FakeConn({"AAPL": ("AAPL", None)})
...
class TestFmpTier:
    ...
    def test_prefers_fmp_over_the_seed_table(self, monkeypatch):
        self._fmp(monkeypatch, [{"companyName": "Apple Computer Co",
                                 "sector": "Electronics", "cik": "0000320193"}])
        conn = FakeConn({"AAPL": ("AAPL", None)})
    ...
    def test_covers_a_ticker_the_seed_table_has_never_heard_of(self, monkeypatch):
        self._fmp(monkeypatch, [{"companyName": "Johnson & Johnson",
                                 "sector": "Healthcare", "cik": None}])
    ...
    def test_falls_back_to_the_seed_when_fmp_errors(self, monkeypatch):
        """FMP reports a bad key as HTTP 200 with an error object."""
        self._fmp(monkeypatch, {"Error Message": "Invalid API KEY"})
        conn = FakeConn({"AAPL": ("AAPL", None)})

        assert company_info.enrich(conn, "AAPL") is True
        assert conn.db["AAPL"]["sector"] == "Technology"  # from the seed tier
...
class TestBackfillAll:
    def test_enriches_only_the_placeholder_rows(self, no_fmp):
        conn = FakeConn({
            "AAPL": ("AAPL", None),                  # placeholder
            "MSFT": ("MSFT", None),                  # placeholder
            "TSLA": ("Tesla, Inc.", "Consumer Cyclical"),  # already good
            "ZZZZ": ("ZZZZ", None),                  # placeholder, unknown ticker
        })
