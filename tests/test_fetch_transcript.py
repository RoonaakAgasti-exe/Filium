...
class TestIngestTranscriptWritesTheRealSchema:
    ...
    def test_chunks_record_the_embedding_model(self, wired):
        ...
        assert "BAAI/bge-small-en-v1.5" in [p[-1] for p in cur.params if p and len(p) == 6]
...
class TestReIngestIsIdempotent:
    ...
    def test_the_delete_precedes_the_chunk_insert(self, wired):
        conn, cur = wired
        ft.ingest_transcript("AAPL", 2024, "Q1", "prose")
        first_delete = next(i for i, s in enumerate(cur.statements) if "DELETE FROM filing_chunks" in s)
        first_insert = next(i for i, s in enumerate(cur.statements) if "INSERT INTO filing_chunks" in s)
